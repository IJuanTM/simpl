...
- **CLI scripts** (wired up as composer commands on install): `composer migrate` / `migrate:fresh`
  / `migrate:rollback`, `composer seed` / `seed:fresh`, `composer cron:test` (or `./simpl migrate`/`migrate:fresh`/`migrate:rollback`/`seed`/`seed:fresh`/`cron:test` if you're using Docker).
