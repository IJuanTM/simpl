 ...
 * AdminPage
 *
 * Central admin panel handling /admin/users/* (user management) and
 * /admin/roles/* (role management). Requires admin role to access.
 */
class AdminPage
{
    ...
    public function __construct(Page $page)
    {
        ...
        match ($this->section) {
            'users' => $this->initUsers($page),
            'roles' => $this->initRoles($page),
            'dashboard' => null,
            default => PageController::redirect('admin/users'),
        };
    }
    ...
    private function initUsers(Page $page): void
    {
        $this->availableRoles = DB::select('*', 'roles', [], null, 'name ASC');
        $this->tableColumns = self::getUserTableColumns();
    ...
    private static function getUserTableColumns(): array
    {
    ...
    private function loadUsers(): void
    {
        ...
        foreach ($this->allUsers as $key => $user) {
            $roleId = DB::single('role_id', 'user_roles', ['user_id' => $user['id']])['role_id'] ?? null;
            $roleName = $roleId ? DB::single('name', 'roles', ['id' => $roleId])['name'] ?? null : null;
    ...
    private function createUser(): void
    {
        ...
        $roleRecord = DB::single('id', 'roles', ['name' => $_POST['role']]);
        if ($roleRecord) DB::insert('user_roles', ['user_id' => $id, 'role_id' => $roleRecord['id']]);
    ...
    private function postUsers(Page $page): void
    {
    ...
    private function updateUser(Page $page): void
    {
        ...
        $roleRecord = DB::single('id', 'roles', ['name' => $_POST['role']]);
        if ($roleRecord) {
            if (DB::exists('user_roles', ['user_id' => $id])) DB::update('user_roles', ['role_id' => $roleRecord['id']], ['user_id' => $id]);
            else DB::insert('user_roles', ['user_id' => $id, 'role_id' => $roleRecord['id']]);
        }
    ...
    {
        DB::update('users', ['is_active' => 0, 'deleted_at' => date('Y-m-d H:i:s')], compact('id'));
    ...
    {
        DB::update('users', ['is_active' => 1, 'deleted_at' => null], compact('id'));

        PageController::redirect('admin/users');
        AlertController::globalAlert('User successfully restored!', AlertType::SUCCESS, 4);
    }

    private function initRoles(Page $page): void
    {
        $this->loadRoles();
        ...
        if (in_array($this->subAction, ['edit', 'delete'])) {
            ...
            $id = (int)$page->param('id');
            $role = DB::single('*', 'roles', compact('id'));
    ...
    private function postRoles(): void
    {
        match ($this->subAction) {
    ...
    private function updateRole(int $id): void
    {
        ...
        $existing = DB::single('id', 'roles', ['name' => $_POST['name']]);
        if ($existing && (int)$existing['id'] !== $id) {
            FormController::addAlert('A role with this name already exists!', AlertType::WARNING);
            return;
        }

        DB::update('roles', ['name' => $_POST['name']], compact('id'));
    ...
    {
        $userCount = (int)DB::count('user_roles', ['role_id' => $id]);
    ...
    private function renderPaginationInfo(): string
    {
        return $this->startIndex . ' - ' . $this->endIndex . ' of ' . $this->totalUsers
            . ($this->search !== '' ? ' / ' . $this->totalAllUsers : '') . ' users';
    }
}
